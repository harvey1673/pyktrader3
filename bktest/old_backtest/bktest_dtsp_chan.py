...
class DTChanSim(StratSim):
    ...
    def run_vec_sim(self):
        ...
        xdf['rng'] = pd.DataFrame([0.2 * xdf['atr'].shift(1), self.k * xdf['tr'].shift(1)]).max()
        xdf['upper'] = xdf['open'] + xdf['rng'] * (1 + ((xdf['open'] < xdf['ma'].shift(1) - 0.0 * xdf['atr'].shift(1)) & self.use_chan) *self.f)
        xdf['lower'] = xdf['open'] - xdf['rng'] * (1 + ((xdf['open'] > xdf['ma'].shift(1) + 0.0 * xdf['atr'].shift(1)) & self.use_chan) *self.f)
