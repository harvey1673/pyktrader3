...
def calc_port_values(fwd_path, fwds, vols, exp_dates, volumes, start_date=0, end_date = 365.0,  \
                     opt_strikes = [63.5, 45.0], ir = 0.02, accr = 365.0):
    ...
    for step in range(num_steps):
        ...
        for path in range(num_paths):
            for idx, (f, v, dexp, vol) in enumerate(zip(fwds, vols, exp_dates, volumes)):
                for idy, s in enumerate(opt_strikes):
                    ...
                    if dexp <= curr_date:
                        ...
                        opt_values[idy][step, path, idx] = bsopt.BSFwd( IsCall, fwd_path[step, path, idx], s, uv, (dexp - curr_date)/accr, ir) * vol
    return opt_values
...
def test_run():
    ...
    opt_vals = calc_port_values(fwd_path, fwds, vols, exp_dates, volumes, start_date, end_date, \
                     opt_strikes = opt_strikes, ir=0.03, accr=accr)
    num_steps += 1
    ...
    for step in range(num_steps):
        ...
        if step == 0:
            ...
            ex_prob = [0.0]
            mpe = [np.mean([ port for port in port_vals[step,:] if port > 0])]
            mne = [np.mean([port for port in port_vals[step, :] if port <= 0])]
        else:
            ...
            ex_prob.append(sum([1.0 for port in port_vals[step, :] if port < -28400])/num_paths)
            mpe.append(np.mean([port for port in port_vals[step, :] if port > 0]))
            mne.append(np.mean([port for port in port_vals[step, :] if port <= 0]))
    ...
    writer = pd.ExcelWriter('C:\\dev\\data\\Tacora_port_181115.xlsx')
    df.to_excel(writer, 'fwd')
    df = pd.DataFrame(port_pct, columns = ['port_' + str(p) for p in pt_list])
    df.to_excel(writer, 'port')
    df = pd.DataFrame(list(zip(*[ex_prob, mpe, mne])), columns = ['prob', 'MPE', 'MNE']).fillna(0.0)
    df.to_excel(writer, 'port_ME')
    writer.save()
