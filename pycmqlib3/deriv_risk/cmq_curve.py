...
class ForwardCurve(object):
    class InterpMode:
        PiecewiseConst = 0
        PWConstLeft = 1
        Linear = 2  # linear on zero rates
        LinearLog = 3  # linear on log discounts
        LinearFlat = 4
        LinearLogFlat = 5
    ...
    def from_array(cls, tenors, forwards, t0=None, interp_mode=InterpMode.PiecewiseConst):
        ...
        elif interp_mode == cls.InterpMode.PWConstLeft:
            tenors_adj = [t0 - tt for tt in tenors]
            interpfwd = interp1d(tenors_adj, forwards, kind='zero', bounds_error=False, fill_value='extrapolate')
            return cls(t0, lambda t: interpfwd(t0 - t))
        ...
        elif interp_mode == cls.InterpMode.LinearFlat:
            interpfwd = interp1d(tenors, forwards, kind='linear', bounds_error=False, fill_value=(forwards[0], forwards[-1]))
            return cls(t0, lambda t: interpfwd(t))
        elif interp_mode == cls.InterpMode.LinearLogFlat:
            linzero = interp1d(tenors, np.log(forwards), kind='linear', bounds_error=False, fill_value=(np.log(forwards[0]), np.log(forwards[-1])))
            return cls(t0, lambda t: np.exp(linzero(t)))
        else:
            raise BaseException('invalid curve interpolation mode ...')
