...
def linelog(msg):   #在同一行覆盖显示日志输出
    sys.stdout.write(str(('\r%s%s' % (msg,' ' * (LINELENGTH - len(msg)))))) #.encode('gbk'))  #适应输出编码为gbk


##这段代码来自于回测平台。根据lvzhl的建议演化而来,主要用于方便统一计算IF和商品的3/5/10/15/30/...等周期数据
##对于1130和最后一个时点的计算与回测平台不同, 为兼容ticks处理
def calc_t2order(begin,end, xxx_todo_changeme=(1130,1300)):
    ##为便于计算30分钟线，商品的节休息段仍然计算序号. 这样，1000->1029算一个30分钟段
    (mid1,mid2) = xxx_todo_changeme
    t2order = {}
...
def quick_id(v):
    ...
    if t == list: #用到最多
        ...
        return v,None
    elif t in [int,bool,complex,str,str,xrange]:
        return v,None
    #其余都为对象，用id标识
    return id(v),v
...
def _icache(func, *args, **kw):
    ...
    '''
    key = args + iter(kw.items()) if kw else args
...
def _indicator(func, *args, **kw):
    ...
    #print 'in _indicator'
    vargs = list(args)
    key = vargs + list(kw.values()) if kw else vargs
    #print vargs
    vkey,vobjs = quick_ids(key)    
