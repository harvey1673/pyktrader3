...
class RsiAtrStrat(Strategy):
    ...
    asset_params = dict({'rsi_win': 14, 'rsi_th': 20.0, 'atrma_win': 10, 'atr_win': 20, \
                         'freq': 'm1', 'stoploss': 4, }, **Strategy.asset_params)
    ...
    def initialize(self):
        ...
        for idx, underlier in enumerate(self.underliers):
            ...
            min_id = int(min_id/100)*60 + min_id % 100 - (int(self.freq[idx][1:]) if len(self.freq[idx]) > 0 else 0)
        ...
        if len(self.submitted_trades[idx]) == 0:
            ...
            for tradepos in self.positions[idx]:
                if (self.close_tday[idx] and (min_id >= self.last_min_id[idx])) \
                    ...
                    self.status_notifier(tradepos.insts[0], msg)
                    return True
                else:
    ...
    def check_trigger(self, idx):
        ...
        save_status = False
        rsi_key = "RSI" + str(self.rsi_win[idx])
        xdf = self.agent.bar_factory[inst][self.freq[idx]].data
        rsi_dir = dh.crossover(xdf[rsi_key], 50 + self.rsi_th[idx], 1) * 1 \
                  - dh.crossover(xdf[rsi_key], 50 - self.rsi_th[idx], -1) * 1
