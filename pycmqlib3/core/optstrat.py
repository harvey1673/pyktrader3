...
class OptionStrategy(object):
    ...
    def __init__(self, config, agent = None):
        ...
        self.underliers = list(self.products.keys())                
        self.option_dict = self.get_option_dict(self.products)
        self.option_insts = list(self.option_dict.values())
    ...
    def update_margin(self):
        for key in ['margin_long', 'margin_short']:
            self.risk_table.data[key] = [ inst.calc_margin_amount(Direction.LONG, price) \
    ...
    def risk_agg(self, risk_list):
        ...
        res = dict([(instID, dict(list(zip(['pv','delta','gamma','vega','theta'], rec)))) \
