import copy
from pycmqlib3.utility.misc import sign, NO_ENTRY_TIME

tradepos_header = ['insts', 'vols', 'pos', 'target_pos', 'entry_price', 'entry_time', 'entry_target', 'entry_tradeid',
                   'exit_price', 'exit_time', 'exit_target', 'exit_tradeid', 'profit', 'is_closed', 'multiple',
                   'reset_margin', 'trailing', 'tag']

class TrailLossType:
    Ratio, Level = list(range(2))
...
class TradePos(object):
    def __init__(self, **kwargs):
        ...
        self.target_pos = kwargs['target_pos']
        self.direction = sign(self.pos) if (self.pos != 0) else 0
    ...
    def update(self, price, vol, start_time):
        if abs(vol) >= abs(self.pos):
            self.entry_price = price
        self.pos += vol
        if self.pos == 0:
            self.is_closed = True
    ...
    def close(self, price, end_time, vol = None):
        ...
        if self.pos + vol != 0:
            ...
            closed_pos.pos = - vol
            closed_pos.direction = sign(closed_pos.pos) if (closed_pos.pos != 0) else 0
            self.pos += vol
            self.direction = sign(self.pos) if (self.pos != 0) else 0
            self.exit_tradeid = 0
        else:
...
def tradepos2dict(tradepos):
    ...
    trade['target_pos'] = tradepos.target_pos
