...
import datetime
import pandas as pd
from pycmqlib3.utility import misc, sec_bits
...
def get_fut_quotes(inst_list, proxies = PROXIES):
    ...
    raw = [ data for data in raw.split('\n') if '=' in data]
    result = dict()
    for instID, exch, raw_data in zip(inst_list, exch_list, raw):
        quotes = raw_data.split('=')[1]
        quotes = quotes[1:-1].split('"')[0]
        if len(quotes) == 0:
            continue
        quotes = quotes.split(',')
        if exch in ['CFFEX',]:
...
def get_fut_hist(instID, freq = 'd', proxies = PROXIES):
    ...
    if date_col == 'date':
        df[date_col] = df['datetime'].apply(lambda x: datetime.datetime.strptime(x, "%Y-%m-%d").date())
    else:
