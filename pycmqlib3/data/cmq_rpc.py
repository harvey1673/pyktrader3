...
class RpcServer(DataPacker):
    ...
    def run(self):
        while self.__active:
            ...
            req = self.unpack(reqb.decode('ascii'))
            ...
            repb = self.pack(rep)
            self.__rep_socket.send(repb.encode('ascii'))

    def publish(self, topic, data):
        datab = self.pack(data)
        self.__pub_socket.send_multipart([topic.encode('ascii'), datab.encode('ascii')])
...
class RpcClient(DataPacker):
    ...
    def __getattr__(self, name):
        def dorpc(*args, **kwargs):
            ...
            reqb = self.pack(req)
            self.__req_socket.send(reqb.encode('ascii'))
            repb = self.__req_socket.recv()
            rep = self.unpack(repb.decode('ascii'))
            if rep[0]:
                return rep[1]
            else:
    ...
    def run(self):
        while self.__active:
            ...
            data = self.unpack(datab.decode('ascii'))
    ...
    def subscribe_topic(self, topic):
        self.__sub_socket.setsockopt(zmq.SUBSCRIBE, topic.encode('ascii'))
...
def test_client():
    req_address = 'tcp://localhost:10030'
    sub_address = 'tcp://localhost:10040'
...
def test_server():
    rep_address = 'tcp://*:10030'
    pub_address = 'tcp://*:10040'
